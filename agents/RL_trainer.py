...
parser.add_argument("--test-id", default = 'test16')
...
path = os.path.join(ESOURCE_PATH , flags.test_id, flags.test_id)
...
def state_to_obs(obss, n):
    return np.array(obss[0:121] + obss[ (121+ 3*n) : (124+3*n)])
...
def action_wrapper(action_dict):
    ...
    for a in range(flags.num_agents):
        action = action_dict[a] # index: agent id
        each = [0] * 5
        each[action] = 1
        action_one_hot = [[each]]
...
for episode in range(start + 1, flags.num_episodes + 1):
    ...
    for a in range(flags.num_agents):
        ...
        agent_obs[a] = state_to_obs(obs, a)
    ...
    for step in range(flags.episode_length):
        ...
        for a in range(flags.num_agents):
            if flags.train:  #and (update_values[0] or done or done['__all__']):
                ...
                agent.step(a, agent_obs_buffer[a], agent_action_buffer[a], rewards[a], agent_obs[a], [done][0]) # TODO done need to updated
                agent_obs_buffer[a] = agent_obs[a].copy()
    ...
    # if episode % 20 == 0:
    #     agent.save('./' + 'RzrqResFile/'+ flags.test_id +  '/navigator_checkpoint'  + '.pth')
